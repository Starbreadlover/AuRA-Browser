...
```text
~/mozilla-source/firefox/obj-*/dist/*.dmg
```
...
Packaged macOS builds are emitted under `obj-*/dist/` as a `.dmg`. Use the packaged `.dmg` to test on another Mac because the raw development `.app` can contain symlinks to the build tree.
